 ...
 */
public class PrenotazioneTimeSeriesChart extends GraficiComposite {
	 ...
	 *            the frame title.
	 */
	public PrenotazioneTimeSeriesChart(String title, Composite parent,
			int style, int tipo) {
		super(parent, style);
			...
			for (Prenotazione item : VisitaDAO.getPrenotazioni(2011)) {
				if (s2.getDataItem(new Day(item.getDataOra())) == null)
					...
					s2.addOrUpdate(new Day(item.getDataOra()),
							s2.getDataItem(new Day(item.getDataOra()))
									.getValue().intValue() + 1);
