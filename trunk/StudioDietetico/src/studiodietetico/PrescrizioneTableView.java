...
public class PrescrizioneTableView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		classVis = new TableForm(top, SWT.BORDER, prescrizioni, "", "",
				PrescrizioneTableView.this, dd, "PrescrizioneTableView");
	...
	public static void aggiungiColonne(TableForm classVis,
		...
		for (int j = 0; j < prescrizioni.size(); j++) {
			nome = ((Prescrizione) prescrizioni.get(j)).getPaziente()
					.getCognome()
					+ " "
					+ ((Prescrizione) prescrizioni.get(j)).getPaziente()
							.getNome();
